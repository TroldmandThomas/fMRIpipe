...
import progressbar as pb #progressbar courtesy of stackoverflow
import sys
import loadmatrix as lm #getting the connectivity matrices from Conn
...
if __name__ == "__main__":
    ...
    thrs = list(map(str, sys.argv[2].strip('[]').split(',')))

    if len(sys.argv) == 4:
        flag = sys.argv[3]
    elif len(sys.argv) != 3:
        print('Usage: (python3.6) obtain_estimates.py (resultsROI.mat) [flags]')
        exit()
    else:
        flag = '--networks'
